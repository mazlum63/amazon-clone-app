...
const carouselSlice = createSlice({
    ...
    reducers: {
        nextBtn: (state) => {
            if (state.carouselPosition >= 5) return
        ...
        carouselTransHandle: (state) => {
            if (state.carouselTransform === `translateX(${-500}%)`) {
            ...
            if (state.carouselTransform === `translateX(${100}%)`) {
                state.carouselPosition = 4
                state.carouselTransform = `translateX(${-400}%)`
                state.carouselTransition = 'none'
            }
        },
        navHandle: (state, action) => {

            state.carouselPosition = action.payload
            state.carouselTransform = `translateX(${-100 * action.payload}%)`
            state.carouselTransition = 'all 0.3s linear 0s'
        }
    }
})
...
export const { nextBtn, prevBtn, carouselTransHandle, navHandle } = carouselSlice.actions
